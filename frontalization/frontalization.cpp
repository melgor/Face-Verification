...
//Fucntion implement Sub2Ind from MatLab
template<typename T>
vector<int>
Sub2Ind(
          int width
                              ...
                              )
{
  ...
  {
    frontalize(images[0],face_rect[i],camera_model[i],i,outFrontal[i]);
  }
  t22 = std::chrono::high_resolution_clock::now();
                                ...
                                , Mat& outFrontal)
{
  ...
  t12 = std::chrono::high_resolution_clock::now();
  frontalize(images,Rect(),camera_model,0,outFrontal);
  outFrontal = outFrontal(_centralFace);
  t22 = std::chrono::high_resolution_clock::now();
  std::cout << "frontalize took "
             << std::chrono::duration_cast<std::chrono::milliseconds>(t22 - t12).count()
             << " milliseconds\n";
                          ...
                          )
{
  ...
  cerr<<"End"<<endl;
  // string sss = to_string(idx) + "ff1.jpg";
  // imwrite(sss,outFrontal);
